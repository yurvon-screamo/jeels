#!/usr/bin/env python3
"""Exploratory: count per-reading frequency in popular_words, print distribution.

Reuses matching helpers from scripts/audit_kanji_readings.py.
Goal: decide a "rare reading" threshold for kanji reading quizzes + UI.
"""
...
build_furigana_lookup = audit.build_furigana_lookup
kata_to_hira = audit.kata_to_hira
word_covers_reading = audit.word_covers_reading
...
def main() -> None:
    ...
    # Build inverted index: kanji_char -> list of words containing it.
    ...
    print(
        f"  Kanji chars referenced in furigana corpus: {len(words_by_kanji)}"
    )

    # Per-reading stats against the FULL corpus (not just popular_words).
    ...
    for entry in kanji_list:
        ...
        for r in entry["on_readings"]:
            ...
            freq = sum(
                ...
                if word_covers_reading(w, kanji, r_hira, "on", furigana_lookup)
            )
        ...
        for r in entry["kun_readings"]:
            ...
            freq = sum(
                1
                for w in corpus_words
                if word_covers_reading(w, kanji, r_hira, "kun", furigana_lookup)
            )
    ...
    print()
    print("=== Overall (corpus = full JmdictFurigana, 218k words) ===")
    ...
    # Top 25 kanji with most rare readings (T=5 as a middle ground)
    print()
    print("=== Top 25 kanji by count of rare readings (freq<=5) ===")
    t = 5
    ranking = []
    for kanji, readings in per_kanji.items():
        rare = [(r, ty, f) for (r, ty, f) in readings if f <= t]
        if rare:
            ranking.append((kanji, len(rare), len(readings), rare))
    ranking.sort(key=lambda x: (-x[1], -x[2]))
    for kanji, n_rare, n_total, rare in ranking[:25]:
        rare_str = ", ".join(f"{r}({ty},f={f})" for (r, ty, f) in rare)
        print(f"  {kanji}: {n_rare}/{n_total} rare  →  {rare_str}")

    # Spotlight: show full breakdown for a few high-readings kanji
    print()
    print("=== Spotlight: readings breakdown for known tricky kanji ===")
    for spotlight in ["生", "上", "下", "分", "日", "人", "気", "中", "大", "子", "行", "出"]:
